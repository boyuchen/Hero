...
(function() {
  var H = DlHighlight = function(args) {
    ...
    if (this.args.replaceTabs != null) {
      var tab = " ";
      while (--this.args.replaceTabs > 0) tab += "&nbsp;"; // 缩进
      this.args.replaceTabs = tab;
    }
  };
  H.is_ie = /MSIE/.test(navigator.userAgent) && !/Gecko|KHTML|Opera/.test(navigator.userAgent); // 判断浏览器
  H.BASE = {
    COMMENT_CPP: function(txt) {
      if (txt.charAt(0) == "/" && txt.charAt(1) == "/") { // 注销
        var nl = txt.indexOf("\n"); // 换行符位置
        if (nl == -1) nl = txt.length; // 没有换行返回字符串长度
        var c = this.lang.onComment.call(this, this._he(txt.substring(2, nl))); // _he 转义方法 ， onComment 方法 把url路径变成A标签
        return {
          content: {
            escaped: c
          },
    ...
    STRING: {
      regexp: /^(\x22(\\.|[^\x22\\])*\x22|\x27(\\.|[^\x27\\])*\x27)/g, // "" 和 '' "ddd" 'ddd'
      content: function(m) {
        m = m[1];
  ...
  H.prototype = {
    ...
    _he: function(str) {
      ...
      str = str.replace(he_re, function(c) { // c 是匹配到的string
        return he_re_val[c];
      });
      if (this.args.replaceTabs) str = str.replace(/\t/g, this.args.replaceTabs); // 制表符替换
      return str;
    }
  };
  var he_re = /[&<>]/g,
